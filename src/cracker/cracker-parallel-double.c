...
int main(int argc, char const *argv[]) {
    ...
    pd_blf_ctx *p_state = get_aligned_pd_state();
    ...
    blowfish_parallelise_state_double(&initstate_parallel_double,
        &initstate_asm);
    ...
    while (!found && bytes_read > 0) {
        ...
        for (size_t j = 0; j < password_groups; ++j) {
            ...
            bcrypt_hashpass_parallel_double(p_state, salt, current_passwords,
            ...
            matching_pass_idx = hash_match_parallel_double(hashes, record_ciphertext);
